    ...
    useEffect(() => {
      if (joystickPosition.x !== 0 || joystickPosition.y !== 0) {
        ...
        if (closestDirectionIndex !== currentDirectionIndex) {
          const currentRing = Math.floor((selectedItem - directionCount / 2) / directionCount);
          const spotInCurrentRing = currentRing * directionCount + closestDirectionIndex;
          const spotInNextRing = (currentRing + 1) * directionCount + closestDirectionIndex;
          setSelectedItem(Math.abs(spotInCurrentRing - selectedItem) < Math.abs(spotInNextRing - selectedItem)
            ? spotInCurrentRing
            : spotInNextRing);
        }
      }
